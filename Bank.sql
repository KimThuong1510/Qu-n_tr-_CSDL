...
select c.*
from customer c
where substring(cust_name,2,1) in ('H','T','A','Ê');
...
select t_id, t_type, t_time
from transactions 
where t_type = 1 
except 
select t_id, t_type, t_time
from transactions 
where t_time between '7:30 ' and '11:30' or  t_time between '13:30'  and '17:30'; 
...
select t_id, t_type, a.ac_balance
from transactions t 
join account a on a.Ac_no = t.Ac_no
where a.ac_balance < 0;
...
select Cust_name
from customer 
where Cust_name not like 'N%' and  Cust_name not like 'T%'; 
---20.Hiển thị danh sách khách hàng có kí tự thứ 3 từ cuối lên là chữ a, u, i
select Cust_name
from customer 
where  Cust_name like '%[a,u,i]__'; 
...
--- 22.Hiển thị khách hàng có địa chỉ sống ở vùng nông thôn. 
...
--- charindex là tìm kiếm chuỗi con trong 1 chuỗi và trả về vị trí 
--- CharIndex( chuoi_con, chuoi_cha, [vi_tri_bat_dau])
--- right là tìm kiểm phần tử bên phải của 1 chuỗi 
--- right(start, số lượng ký tự) 
---- SubString là lấy giá trị chuỗi con đã được chỉ thị 
---SUBSTRING(string, start, length)

select cust_name, right(cust_name, CharIndex(' ', reverse(cust_name)) - 1) As Name 
from customer
where substring(right(cust_name, CharIndex(' ', reverse(cust_name)) - 1), 2,1) in ('u','ũ','i');


select cust_name, CharIndex(' ', reverse(cust_name))-1  As Name 
from customer
